//
//  OLDLevelFinishViewController.m
//  ADTestLabs
//
...
@interface OLDLevelFinishViewController ()
...
@implementation OLDLevelFinishViewController
...
- (void)CompleteQuestionScore
{
    ...
    if (score >=4)
    {
        reportText = @"被测试者的认知功能可能存在问题，建议尽快去专业医生处就诊咨询！";
        
    }
    else
    {
        reportText = @"被测试者目前没有痴呆的早期常见症状！";
    }
    ...
    if (_appDelegate.user != nil)
    {
        [_scaleReportBLL AddScaleReport:_appDelegate.user
         .userID withAnswer:nil withScore:score withScaleReportDesc:reportText withScaleLevelTag:ScaleLevelOLD];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"OLDLevelToShowHospitalLocationMapView"])
    {
